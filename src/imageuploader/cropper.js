...
(function(factory) {
    ...
    Cropper.prototype = {
        ...
        setDragger: function() {
            ...
            this.$image.trigger("ready.cropper");
        },
        ...
        resetDragger: function() {
            ...
            if (dragger.width > dragger.maxWidth || dragger.width < dragger.minWidth) {
                dragger.left = dragger.oldLeft;
            }

            if (dragger.height > dragger.maxHeight || dragger.height < dragger.minHeight) {
                dragger.top = dragger.oldTop;
            }

            dragger.width = Math.min(Math.max(dragger.width, dragger.minWidth), dragger.maxWidth);
            dragger.height = Math.min(Math.max(dragger.height, dragger.minHeight), dragger.maxHeight);

            dragger.maxLeft = cropper.width - dragger.width;
            dragger.maxTop = cropper.height - dragger.height;

            dragger.oldLeft = dragger.left = Math.min(Math.max(dragger.left, 0), dragger.maxLeft);
            dragger.oldTop = dragger.top = Math.min(Math.max(dragger.top, 0), dragger.maxTop);
        ...
        setImgSrc: function(src) {
            if (typeof src === "string" && src.length > 0 && src !== this.src) {
                ...
                this.$image.trigger("ready.duplicate");
            }
        },
